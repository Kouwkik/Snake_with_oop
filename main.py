...
cell_size = 25 # высота игровой сетки
number_of_cells = 20 # ширина игровой сетки

OFFSET = 75

class Food:
    def __init__(self, snake_body):
        self.position = self.generate_random_pos(snake_body) # стартовая позиция еды


    def draw(self):
        food_rect = pygame.Rect(OFFSET + self.position.x * cell_size, OFFSET + self.position.y * cell_size, cell_size, cell_size)
        screen.blit(food_surface, food_rect) # отображение еды на экране
...
class Game:
    ...
    def draw(self):
        self.food.draw() # создание объекта с изображением еды
        self.snake.draw() # создание объекта с изображением змеи
...
screen = pygame.display.set_mode((2 * OFFSET + cell_size * number_of_cells, 2 * OFFSET + cell_size * number_of_cells)) # создание экрана с игровой сеткой
...
food_surface = pygame.image.load("apple-119.png")
...
while True:
    for event in pygame.event.get(): # получаем все события с момента последнего запуска и помещаем в список
        if event.type == SNAKE_UPDATE:
            game.update() # обновление координат змеи
        if event.type == pygame.QUIT: # если нажали крестик - конец игры
            pygame.quit()
            sys.exit()

        if event.type == pygame.KEYDOWN: # управление змеей стрелками
